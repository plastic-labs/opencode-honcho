...
- `@opencode/plugin` and `zod` are the plugin's dependencies; the 2.x path no longer touches `@opencode-ai/plugin`, which is now a type-only devDependency for the 1.x adapter. Neither OpenCode package is imported at runtime. Build the bundles for the Node target so they run under both the Bun and Node OpenCode runtimes. 2.x plugin logs go to stderr, never stdout (the server may use stdout as its RPC transport).
