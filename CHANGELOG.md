...
- Send telemetry headers on every Honcho request: `X-Honcho-Host: opencode`, `X-Honcho-Plugin`, `X-Honcho-Runtime`, and `X-Honcho-Agent-Model` (the OpenCode model reported by chat hooks, kept current on the reused client). `honcho_status` reports `enabled`, `timeoutMs`, `configWarnings`, and `telemetry`.
