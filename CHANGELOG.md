...
- Send telemetry headers on every Honcho request: `X-Honcho-Host` (`opencode[/<version>] (<platform>)`), `X-Honcho-Plugin` (`opencode-honcho/<version>`), and `X-Honcho-Agent-Model` (`providerID/modelID` of the OpenCode model reported by chat hooks, kept current on the reused client). `honcho_status` reports `enabled`, `timeoutMs`, `configWarnings`, and `telemetry`.
