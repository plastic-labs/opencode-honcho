...
bun install
bun run build
opencode plugin add "$PWD"                    # OpenCode 2.x
opencode plugin "$PWD" --global --force       # OpenCode 1.x
```

OpenCode 2.x resolves a local plugin directory by its root `server` and `tui` modules rather than the `exports` map, so the checkout ships `server.js` and `tui.js` shims that re-export `dist/`. Run `bun run build` before starting OpenCode. Plugin logs go to stderr on 2.x (`opencode run --standalone --print-logs`) and to OpenCode's log under `service=opencode-honcho` on 1.x.
